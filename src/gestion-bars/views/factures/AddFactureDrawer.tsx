...
const schema = yup.object().shape({
  ...
  client: yup.string().required(() => 'Le champ client est obligatoire'),
})
...
const SidebarAddFacture = (props: SidebarAddFactureType) => {
  // ** Props
  const { open, toggle, onEdit, onSuccess, currentFacture } = props
  ...
  const onSubmit = async (data: ModelData) => {
    ...
    if (id != -1) {
      factureService.updateFacture({ ...sendData, id }, id).then(rep => {
        setSend(false)
        if (rep) {
          onEdit()
          reset()
  ...
  useEffect(() => {
    reset({
      ...
      client: currentFacture !== null ? currentFacture?.client : '',
